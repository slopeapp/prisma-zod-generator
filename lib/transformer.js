    ...
    async printModelSchemas() {
        for (const model of this.modelOperations) {
            ...
            if (findUnique) {
                ...
                await (0, writeFileSafely_1.writeFileSafely)(path_1.default.join(Transformer.outputPath, `schemas/${findUnique}.schema.ts`), `${this.getImportsForSchemas(imports)}${this.addExportSchema(`z.object({ where: ${modelName}WhereUniqueInputObjectSchema, include: z.any().optional() })`, `${modelName}FindUnique`)}`);
            }
            if (findFirst) {
                const imports = [
